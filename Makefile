...
build-covariate-drift-detector:
	...
	do
		docker buildx build --platform linux/amd64,linux/arm64 -t $(repository)/covariate-drift-detection-api-$${dataset} --build-arg DATASET=$${dataset} --build-arg PARENT_DIR=detector_api -f detector_api/Dockerfile --push .
	done
...
run-covariate-drift-detector-mnist:
	docker run --name covariate-drift-detection-mnist -p 5001:8000 $(repository)/covariate-drift-detection-api-mnist

run-covariate-drift-detector-fashion-mnist:
	docker run --name covariate-drift-detection-fashion_mnist -p 5001:8000 $(repository)/covariate-drift-detection-api-fashion-mnist

run-covariate-drift-detector-cifar10:
	docker run --name covariate-drift-detection-cifar10 -p 5001:8000 $(repository)/covariate-drift-detection-api-cifar10
...
run-dimensionality-reduction-fashion-mnist:
	docker run --name dimensionality-reduction-fashion-mnist -p 5002:8000 $(repository)/dimensionality-reduction-api-fashion-mnist
...
run-model-inference-fashion-mnist:
	docker run --name model-inference-fashion-mnist -p 5003:8000 $(repository)/model-inference-api-fashion-mnist
